...
class MultimodalJudgmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Removed timing prints to reduce console I/O overhead
        ...
        # Load image directly
        # The bottleneck is usually the processor(), not the disk read.
        # With num_workers > 0, this happens in background threads.
        try:
        ...
        # Optimized label finding (minor speedup)
...
class MultimodalDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        if len(features) == 0:
            raise ValueError("Empty batch received during data loading")

        # 1. Find the longest sequence IN THIS BATCH
        # We do not force it to be CONFIG['max_length'] (768).
        # We let it be whatever the data requires.
        ...
        for feature in features:
            ...
            # This ensures the model IGNORES the padding, preserving 100% accuracy
            padded_attention_mask = torch.cat([
                feature["attention_mask"],
                torch.zeros(pad_len, dtype=torch.long)
            ])

            # Pad labels (-100 to ignore in loss calculation)
        ...
        # Handle images (standard stacking)
...
def train_model(model, processor, train_dataset, dev_dataset, config):
    ...
    # ==== GENERATION-BASED EVALUATION (for final accuracy) ====
    def evaluate_with_generation(eval_dataset, num_samples):
        ...
        with torch.no_grad():
            for idx in range(min(num_samples, len(eval_dataset))):
                ...
                user_length = (true_labels == -100).sum().item()
